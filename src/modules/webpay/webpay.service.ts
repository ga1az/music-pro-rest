...
export class WebpayService {
        ...
        const urlToken = fetch(
            'https://webpay3gint.transbank.cl/rswebpaytransaction/api/webpay/v1.2/transactions',
            {
                ...
                headers: {
                    ...
                    'Tbk-Api-Key-Id':
                        this.configService.get<string>('TBK_API_KEY_ID'),
                    'Tbk-Api-Key-Secret':
                        this.configService.get<string>('TBK_API_KEY_SECRET'),
                },
                body: JSON.stringify({
                    buy_order: buyOrder,
    ...
    async commitTransaction(
        buy_order: string,
        token: string,
    ): Promise<ResponseCommitTransactionDto> {
        const response: ResponseCommitDto = await fetch(
    ...
    async createPreTransaction(
        buyOrder: string,
    ): Promise<ResposeCreatePreTransactionDto> {
        const order = await this.orderService.findByBuyOrder(buyOrder);
        if (!order) {
            throw new NotFoundException(
                `Order with buyOrder ${buyOrder} not found`,
            );
        } else if (order.status !== 'PENDING') {
            throw new NotFoundException(
                `Order with buyOrder ${buyOrder} is not pending`,
            );
        }
